...
def flocking_behavior(environment_data: dict) -> List[Fish]:
    ...
    def separation(boid: Fish, flockmates: List[Fish]) -> Vector2:
        ...
        return  -1 *reduce(lambda a, b: a + b, [mate.position - boid.position for mate in flockmates])  / dtime
    ...
    def alignment(boid: Fish, flockmates: List[Fish]) -> Vector2:
        ...
        return (reduce(lambda a, b: a+b, mates_velocities) / len(flockmates) - boid.velocity) / dtime

    def compute_flocking(boid: Fish) -> Vector2:
        '''Accumulates all traits for one boid'''
        flockmates = find_flockmates(boid)
        if len(flockmates) > 0:
            return cohesion_coefficient * cohesion(boid, flockmates) + alignment_coefficient * alignment(boid, flockmates)
            +  separation_coefficient * \
                separation(boid, find_safe_space_intruders(boid))
        else:
            return Vector2(0, 0)
